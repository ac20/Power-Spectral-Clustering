...
# @profile
def PRcut(graph, k = 6, discrete_Method = 'kmeans', epsilon = 0.01, nClust = 3, verbose = False):
    ...
    if nClust == 1:
        L = laplacian(graph)
        eigval, embedNcut = eigsh(L, k=k, sigma = 1e-6 )
    ...
    u = np.arange(sizeGraph, dtype = np.int32)
    v = np.array(label_components, dtype = np.int32)
    z = Counter(v)
    w = 1./np.sqrt(np.array([z[i] for i in v], dtype = np.float64))
    N = csr_matrix((w,(u,v)),shape=(sizeGraph,n_components))
