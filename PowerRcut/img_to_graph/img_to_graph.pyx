...
def method1_Binarization(int[:,:] im, int s0, int s1, int spat_distance = 1, double beta = 1., double eps = 1e-5, int white = 255, int black = 0 ):
    """
    method1(im, s0, s1, s2, spat_distance, beta, eps)
    ...
    Parameters
    -----------
    im : Image to be converted to graph. Should be int32. Should be a geyscale image.
    s0, s1 : Spatial dimensions of the image.
    s2 : Number of bands
    spat_distance : The distance to which the edges must be added. Default: 1 for 4-adjacency.
    beta, eps : Parameters used for taking similarity. Default : (1., 1e-5) [exp(-beta*data/data.stdev()) + eps]
    ...
    """
    cdef int size = s0*s1*(spat_distance+1)*(spat_distance+1)*2 + 2*s0*s1 + 10
    cdef int[:] mv_ind1 = np.zeros(size, dtype = np.int32), mv_ind2 = np.zeros(size, dtype = np.int32)
    cdef double[:] mv_wSpat = np.zeros(size, dtype = np.float64), mv_wInt = np.zeros(size, dtype = np.float64)
    cdef int[:] mv_wBucket = np.zeros(size, dtype = np.int32)
    cdef int x1 = 0, x2 = 0, y1 = 0, y2 = 0, ind1, ind2, i, ind = 0, w3
                    ...
                    for y2 in range(y1-spat_distance, y1+spat_distance+1):
                        if x2 >= 0 and x2 < s0 and y2 >= 0 and y2 < s1 and (x1 != x2 or y1 != y2):
                            if abs(x1-x2) + abs(y1-y2) <= spat_distance:
                                ind1 = x1*s1 + y1
                                ind2 = x2*s1 + y2
                                w1 = <float> (np.abs(im[x1,y1] - im[x2,y2])/255.)
                                mv_ind1[ind] = ind1
                                mv_ind2[ind] = ind2
                                mv_wInt[ind] = w1
                                ind += 1

                # index of white node (255) s0*s1
                ind1 = x1*s1 + y1
                ind2 = s0*s1
                w1 = <float> (np.abs(im[x1,y1] - white)/255.)
                if (np.abs(im[x1,y1] - white) < 10):
                    mv_ind1[ind] = ind1
                    mv_ind2[ind] = ind2
                    mv_wInt[ind] = w1
                    ind += 1

                # index of  black node (0) s0*s1+1
                ind1 = x1*s1 + y1
                ind2 = s0*s1 + 1
                w1 = <float> (np.abs(im[x1,y1] - black)/255.)
                if (np.abs(im[x1,y1] - black) < 10):
                    mv_ind1[ind] = ind1
                    mv_ind2[ind] = ind2
                    mv_wInt[ind] = w1
                    ind += 1
    ...
    graph_int = csr_matrix((weight_int, (list_1, list_2)), shape = (s0*s1+2, s0*s1+2))
    graph_int.data = np.exp(-beta*(graph_int.data)/(graph_int.data.std())) + eps
    return graph_int, None
