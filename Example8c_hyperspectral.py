...
for scale in np.arange(0.7,0.99,0.1):
    ...
    st_time = time.clock()
    img = loadmat("./Hyperspectral_data/Salinas_corrected.mat")['salinas_corrected']
    s0,s1,s2 = img.shape
    img = np.array(img[:int(scale*s0),:int(scale*s1),:],np.int32)

    img_gt = loadmat("./Hyperspectral_data/Salinas_gt.mat")['salinas_gt']
...
plt.imsave("./img/salinas_gt.pdf",img_gt)
plt.imsave("./img/salinas_PRcut.pdf",np.array(labelsPRcut).reshape(img_gt.shape))
plt.imsave("./img/salinas_Rcut.pdf",np.array(labelsRcut).reshape(img_gt.shape))

from scipy.ndimage.filters import gaussian_filter

filtered_PRcut = gaussian_filter(labelsPRcut.reshape(img_gt.shape), 2)
plt.imsave("./img/salinas_PRcutFiltered.pdf",filtered_PRcut)

from process_small_clusters import *
embedPRcut_filtered = process_small_clusters(embedPRcut,label_components,graph,100)
...
plt.imsave("./img/salinas_embedFilter.pdf",np.array(labelsPRcut_filtered).reshape(img_gt.shape))
