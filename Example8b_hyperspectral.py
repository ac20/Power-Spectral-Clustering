...
"""
Calculating the timing statistics for different subimages of Pavi Center Dataset
---------------------------------------------------------------------------------
"""
...
for scale in np.arange(0.7,0.99,0.1):
    ...
    st_time = time.clock()
    img = loadmat("./Hyperspectral_data/Pavia.mat")['pavia']
    s0,s1,s2 = img.shape
    img = np.array(img[:int(scale*s0),:int(scale*s1),:],np.int32)

    img_gt = loadmat("./Hyperspectral_data/Pavia_gt.mat")['pavia_gt']
    ...
    for _ in range(10):
        ...
        st_time = time.clock()
        L = laplacian(graph)
        eigval, embedRcut = eigsh(L, 50, sigma = 1e-6 )
        ...
        st_time = time.clock()
        embedPRcut, label_components = PRcut(graph, 50, discrete_Method = 'kmeans', nClust=3, threshComp=100, verbose=True)
...
eigval, embedRcut = eigsh(L, k=50, sigma = 1e-6 )
d0, labelsRcut, d2 = k_means(embedRcut,50)
Rcut_time = time.clock() - st_time
...
plt.imsave("./img/paviaCenter_gt.pdf",img_gt)
plt.imsave("./img/paviaCenter_PRcut.pdf",np.array(labelsPRcut).reshape(img_gt.shape))
plt.imsave("./img/paviaCenter_Rcut.pdf",np.array(labelsRcut).reshape(img_gt.shape))

from scipy.ndimage.filters import gaussian_filter

filtered_PRcut = gaussian_filter(labelsPRcut.reshape(img_gt.shape), 2)
plt.imsave("./img/paviaCenter_PRcutFiltered.pdf",filtered_PRcut)

from process_small_clusters import *
embedPRcut_filtered = process_small_clusters(embedPRcut,label_components,graph,50)
d0, labelsPRcut_filtered, d2 = k_means(embedPRcut_filtered,50)
...
plt.imsave("./img/paviaCenter_embedFilter.pdf",np.array(labelsPRcut_filtered).reshape(img_gt.shape))
